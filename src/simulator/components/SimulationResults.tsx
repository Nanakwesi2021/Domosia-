...
import { type SimulationResult, type PropertyInput, CITIES, FURNITURE_QUALITY, validateResults } from "@/simulator/lib/moroccan-market-data";
...
import { supabase } from "@/simulator/integrations/supabase/client";
...
function fmt(n: number, currentLang: string = "fr") {
  const locale = currentLang === "en" ? "en-US" : "fr-MA";
    ...
    <div className="w-full max-w-4xl mx-auto space-y-8" dir={dir}>
      ...
      <div ref={page1Ref} className="space-y-6 bg-[#faf8f5] p-4">
          ...
          <div className="flex items-start justify-between flex-wrap gap-4">
            <div>
              <p className="text-sm text-muted-foreground font-medium">
                {propertyLabel}
                {" · "}{tr(`city.${input.city}`)}{input.neighborhood ? `, ${tr(`neighborhood.${input.neighborhood}`)}` : ""}
                {" · "}{input.bedrooms} {tr("property.bedrooms").toLowerCase()}
              </p>
              <h2 className="text-3xl font-serif mt-2" style={{ lineHeight: "1.15" }}>
                {fmt(result.annualRevenue, lang)} <span className="text-lg text-muted-foreground">{tr("results.annualRevenue")}</span>
              </h2>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4" style={{ animationDelay: "100ms" }}>
          {[
            { icon: Sun, label: tr("results.highSeason"), value: `${fmt(result.nightlyRateHigh.min, lang)}–${fmt(result.nightlyRateHigh.max, lang)}`, sub: tr("common.madPerNight") },
            { icon: Cloud, label: tr("results.lowSeason"), value: `${fmt(result.nightlyRateLow.min, lang)}–${fmt(result.nightlyRateLow.max, lang)}`, sub: tr("common.madPerNight") },
            { icon: Calendar, label: tr("results.occupancy"), value: `${result.occupancyHigh}% / ${result.occupancyLow}%`, sub: tr("results.highLow") },
            { icon: TrendingUp, label: tr("results.longTermRevenue"), value: fmt(result.annualLongTerm, lang), sub: tr("common.madPerYear") },
          ].map((m, i) => (
            <div
              key={m.label}
            ...
            <table className="w-full text-sm">
              ...
              <tbody>
                <tr className="border-b">
                  ...
                  <td className="text-right tabular-nums font-medium">{fmt(result.monthlyRevenueHigh, lang)} {tr("common.mad")}</td>
                  <td className="text-right tabular-nums text-muted-foreground">{fmt(result.monthlyRevenueHigh * 6, lang)} {tr("common.mad")}</td>
                </tr>
                <tr className="border-b">
                  <td className="py-3 font-medium flex items-center gap-2"><Cloud className="w-4 h-4 text-chart-lt" /> {tr("results.lowSeasonLabel")}</td>
                  <td className="text-right tabular-nums font-medium">{fmt(result.monthlyRevenueLow, lang)} {tr("common.mad")}</td>
                  <td className="text-right tabular-nums text-muted-foreground">{fmt(result.monthlyRevenueLow * 6, lang)} {tr("common.mad")}</td>
                </tr>
                <tr className="bg-primary/5 rounded">
                  <td className="py-3 font-semibold">{tr("results.annualEstimate")}</td>
                  <td></td>
                  <td className="text-right tabular-nums font-bold text-primary">{fmt(result.annualRevenue, lang)} {tr("common.mad")}</td>
                </tr>
              </tbody>
            </table>
          </div>
        ...
        <div className="grid md:grid-cols-2 gap-4 animate-fade-up" style={{ animationDelay: "400ms" }}>
          <div className="bg-accent/10 rounded-xl border border-accent/20 p-6">
            ...
            <p className="text-2xl font-serif tabular-nums" style={{ lineHeight: "1.15" }}>{fmt(result.bestCase, lang)} {tr("common.madPerYear")}</p>
          ...
          <div className="bg-muted rounded-xl border p-6">
            ...
            <p className="text-2xl font-serif tabular-nums" style={{ lineHeight: "1.15" }}>{fmt(result.worstCase, lang)} {tr("common.madPerYear")}</p>
