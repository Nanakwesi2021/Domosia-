  ...
  if (limits) {
    if (input.bedrooms > limits.maxBedrooms) {
      warnings.push({ 
        field: "bedrooms", 
        key: "val.maxBedrooms", 
        params: { count: limits.maxBedrooms, type: `property.${input.propertyType}` } 
      });
    }
    if (input.beds > limits.maxBeds) {
      warnings.push({ 
        field: "beds", 
        key: "val.maxBeds", 
        params: { count: limits.maxBeds, type: `property.${input.propertyType}` } 
      });
    }
    if (input.bathrooms > limits.maxBathrooms) {
      warnings.push({ 
        field: "bathrooms", 
        key: "val.maxBathrooms", 
        params: { count: limits.maxBathrooms, type: `property.${input.propertyType}` } 
      });
    }
  }
